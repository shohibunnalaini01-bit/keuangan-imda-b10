...
// === FUNGSI PENGECEKAN BOOLEAN YANG AMAN ===
const isTruthy = (val) => {
    return val === true || val === 1 || val === 'true' || val === '1';
};

// === HELPER GENDER (KEBAL SEMUA FORMAT! ===
// Menangani: "Laki-laki", "LAKI - LAKI", "LAKI-LAKI", "Perempuan", "PEREMPUAN", dll
function isFemale(g) {
    ...
    if(students) {
        students.forEach(st => {
            if(st.class >= 1 && st.class <= 6) {
                ...
                // Gunakan helper gender, bukan perbandingan string langsung
...
async function loadStudents() {
    ...
    document.getElementById('imdaTitle').innerHTML = `<i class="fas fa-file-invoice-dollar"></i> Kelas ${currentClass}${titleGenderText} - Pembayaran IMDA`;
    ...
    if(students) {
        // Tentukan jenis kelamin yang dicari dari dropdown
        ...
        students.forEach(st => {
            // FILTER GENDER AMAN: bandingkan hasil isFemale(), bukan string mentah
            if(currentGender) {
                let stIsFemale = isFemale(st.gender);
                if(wantFemale !== stIsFemale) return; // lewati yang tidak cocok
            }
            ...
            tbody.innerHTML += `<tr>
                ...
                <td>${st.name}</td>
                <td style="text-align: center; font-weight: 500;">${genderShort(st.gender)}</td>
                <td style="color: ${isPaid ? 'var(--success)' : 'var(--danger)'}; font-weight:600;">${isPaid ? 'Lunas' : 'Belum Bayar'}</td>
            </tr>`;
        });

        if(tbody.innerHTML === '') {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; color:var(--text-muted)">Tidak ada murid yang cocok dengan filter</td></tr>`;
        }
    }
}
